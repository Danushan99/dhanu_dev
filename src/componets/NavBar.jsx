...
const NavBar = () => {
  ...
  return (
    ...
    <div
      className={`w-full ${isSticky ? "bg-home" : "bg-home"
    ...
    >
      <nav className='py-4 lg:px-14 px-4 ${isSticky ? "sticky top-0 right-0 left-0 border-b bg-gray-4duration-300 : "" }'>
        ...
        {/* border-b-2 border-black  */}
          ...
          >
            <span className="text-3xl font-Butler-regular ">Danushan Ravendran</span>
          </a>
        ...
        <div
          className={
            `space-y-4 px-4 mt-16 py-7 bg-white ${ isMenuOpen ? "block fixed top-0 right-0 left-0" : "hidden" }`
          } >
          {navItems.map(({ link, path }) => (
            <li
              key={path}
